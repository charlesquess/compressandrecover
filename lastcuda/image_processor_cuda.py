...
class ImageProcessor:
    ...
    def process_image(self):
        ...
        # 计算压缩后总占用空间大小
        print(f"Memory size of compressed_x+compressed_y+compressed_z: {sys.getsizeof(compressed_x)+sys.getsizeof(compressed_y)+sys.getsizeof(compressed_z)} bytes")
        # 查看压缩后的数据长度
        print(f"Length of compressed_x+compressed_y+compressed_z: {len(compressed_x)+len(compressed_y)+len(compressed_z)} bytes")
