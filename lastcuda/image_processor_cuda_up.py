...
class ImageProcessor:
    ...
    """
    def __init__(self, image_path, N=4):
        """
        初始化ImageProcessor对象
    ...
    def process_image(self):
        ...
        # 计算压缩前的坐标数据所占用的空间大小
        print(f"Memory size of x_block: {sys.getsizeof(x_block)} bytes")
        print(f"Memory size of y_block: {sys.getsizeof(y_block)} bytes")
        print(f"Memory size of z_block: {sys.getsizeof(z_block)} bytes")
        print(f"Memory size of x_local: {sys.getsizeof(x_local)} bytes")
        print(f"Memory size of y_local: {sys.getsizeof(y_local)} bytes")
        print(f"Memory size of z_local: {sys.getsizeof(z_local)} bytes")
        ...
        # 计算压缩后的坐标数据所占用的空间大小
        print(f"Memory size of compoessed_x_block: {sys.getsizeof(compoessed_x_block)} bytes")
        print(f"Memory size of compoessed_y_block: {sys.getsizeof(compoessed_y_block)} bytes")
        print(f"Memory size of compoessed_z_block: {sys.getsizeof(compoessed_z_block)} bytes")
        print(f"Memory size of compoessed_x_local: {sys.getsizeof(compoessed_x_local)} bytes")
        print(f"Memory size of compoessed_y_local: {sys.getsizeof(compoessed_y_local)} bytes")
        print(f"Memory size of compoessed_z_local: {sys.getsizeof(compoessed_z_local)} bytes")

        # # 执行RLE解压，将压缩后的坐标数据解压回原来的坐标形式
        # decoded_x_block = self.rle_compressor.rle_decode(compoessed_x_block)
        # decoded_y_block = self.rle_compressor.rle_decode(compoessed_y_block)
        # decoded_z_block = self.rle_compressor.rle_decode(compoessed_z_block)
        # decoded_x_local = self.rle_compressor.rle_decode(compoessed_x_local)
        # decoded_y_local = self.rle_compressor.rle_decode(compoessed_y_local)
        # decoded_z_local = self.rle_compressor.rle_decode(compoessed_z_local)
