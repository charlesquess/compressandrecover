...
class ImageProcessor:
    ...
    def rgb_to_coordinates(self):
        """将RGB值转换为坐标并记录日志"""
        # 初始化坐标数组
        x_relative = np.zeros((self.height, self.width), dtype=np.float32)
        y_relative = np.zeros((self.height, self.width), dtype=np.float32)
        z_relative = np.zeros((self.height, self.width), dtype=np.float32)
